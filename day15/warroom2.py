...
class WarRoom(object):
    ...
    def __init__(self, fpath, elf_ap):
        self.rounds = 0
        self.warriors = []
        self.room_map = []
        for r, line in enumerate(open(fpath)):
            ...
            for c, char in enumerate(line):
                if char in 'EG':
                    self.warriors.append(Warrior(r, c, char, self, elf_ap))
                    room_line.append('.')
                elif char == '\n':
                    pass
                else:
...
class Warrior(object):
    def __init__(self, r, c, race, warroom, elf_ap):
        self.pos = (r, c)
        self.race = race
        self.warroom = warroom
        self.hit_points = 200
        if race == 'E':
            self.attack_power = elf_ap
        else:
            self.attack_power = 3
        self.had_turn = False
...
elf_ap = 4
while True:
    wr = WarRoom(fpath, elf_ap)
    starting_elves = sum(1 for w in wr.warriors if w.race == 'E')
    while wr.still_fighting():
        wr.take_round()
    if starting_elves == sum(1 for w in wr.warriors if w.race == 'E'):
        break
    elf_ap += 1
    print 'New Elf AP:', elf_ap
